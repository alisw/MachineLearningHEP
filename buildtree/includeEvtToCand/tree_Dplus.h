...
// from TTree tree_Dplus/tree_Dplus
...
#ifdef tree_Dplus_cxx
tree_Dplus::tree_Dplus(TTree *tree) : fChain(0)
{
...
//   if (tree == 0) {
      ...
      // }
      //f->GetObject("tree_Dplus",tree);

      //}
   Init(tree);
}

tree_Dplus::~tree_Dplus()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t tree_Dplus::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Int_t tree_Dplus::GetEntriesFast()
{
  // Read contents of entry.
  if (!fChain) return 0;
  return fChain->GetEntriesFast();
}
Long64_t tree_Dplus::LoadTree(Long64_t entry)
{
...
void tree_Dplus::Init(TTree *tree)
{
...
void tree_Dplus::Show(Long64_t entry)
{
...
Int_t tree_Dplus::Cut(Long64_t entry)
{
