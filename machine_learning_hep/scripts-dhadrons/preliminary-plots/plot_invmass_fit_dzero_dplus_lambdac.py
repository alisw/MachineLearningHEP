...
# import or copy the file O2Physics/PWGHF/D2H/Macros/style_formatter.py
...
def main(particle, i_pt, cfg, batch):
    ...
    set_global_style(padtopmargin=0.07, padleftmargin=0.14, padbottommargin=0.125,
                     titleoffsety=1.3, titleoffsetx=1., maxdigits=3)
    ...
    legend = TLegend(0.6, 0.54, 0.87, 0.75) if particle == D0 else TLegend(0.62, 0.58, 0.85, 0.72)
    ...
    frame_promptEnhanced = pad1.DrawFrame(mass_mins[i_pt], ymin_promptEnhanced,
                                          mass_maxs[i_pt], ymax_promptEnhanced, title)
    ...
    lat_label.DrawLatex(0.19, 0.79, "pp,#kern[-0.08]{ #sqrt{#it{s}} = 13.6 TeV,}" \
                        "#kern[-0.08]{ #it{L}_{int} = 5 pb^{#minus1}}")
...
if __name__ == "__main__":
    ...
    for ipt in range(len(configuration["pp13.6TeVFD"]["PtMin"])):
        main(particle=LAMBDAC_TO_PKPI, i_pt=ipt, cfg=configuration, batch=args.batch)
