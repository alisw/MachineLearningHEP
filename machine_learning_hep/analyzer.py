...
class Analyzer:
    ...
    def studyevents(self):
        ...
        cv0m = TCanvas('cv0m_norm', 'V0M / MB')
        leg_v0m = TLegend(.5, .65, .7, .85)
        leg_v0m.SetBorderSize(0)
        leg_v0m.SetFillColor(0)
        leg_v0m.SetFillStyle(0)
        leg_v0m.SetTextFont(42)
        leg_v0m.SetTextSize(0.035)
        href = filedata.Get("hnum%s" % labelsv0_norm[0])
        for i, lab in enumerate(labelsv0_norm[1:]):
            print(lab)
            heff = filedata.Get("hnum%s" % lab)
            heff.SetLineColor(i+1)
            heff.Divide(heff, href, 1.0, 1.0, "B")
            heff.GetXaxis().SetTitle("offline V0M")
            heff.GetYaxis().SetTitle("VHM / MB")
            heff.Draw("epsame")
        ...
        cspd = TCanvas('cspd_norm', 'SHM / MB')
        leg_spd = TLegend(.5, .65, .7, .85)
        leg_spd.SetBorderSize(0)
        leg_spd.SetFillColor(0)
        leg_spd.SetFillStyle(0)
        leg_spd.SetTextFont(42)
        leg_spd.SetTextSize(0.035)
        href = filedata.Get("hnum%s" % labelsspd_norm[0])
        for i, lab in enumerate(labelsspd_norm[1:]):
            print(lab)
            heff = filedata.Get("hnum%s" % lab)
            heff.SetLineColor(i+1)
            heff.Divide(heff, href, 1.0, 1.0, "B")
            heff.GetXaxis().SetTitle("SPD tracklets")
            heff.GetYaxis().SetTitle("SHM / MB")
            heff.Draw("epsame")
        ...
        leg_spd.SetTextFont(42)
        leg_spd.SetTextSize(0.035)
        for i, lab in enumerate(labelsspd[1:3]):
            heff = filedata.Get("hnum%s" % lab)
            hden = filedata.Get("hden%s" % lab)
            heff.SetLineColor(i+1)
            # heff.Divide(heff, hden, 1.0, 1.0, "B")
