...
#
    ...
    def jetsystematics(self):
        ...
        for ibin2 in range(self.p_nbin2_gen):
            ...
            latex = TLatex(0.18,0.85,"ALICE Preliminary, pp, #sqrt{#it{s}} = 13 TeV")
            draw_latex(latex)
            latex1 = TLatex(0.18,0.8,"#Lambda_{c}^{#plus} (and charge conj.) in charged jets, anti-#it{k}_{T}, #it{R} = 0.4, #left|#it{#eta}_{jet}#right| < 0.5")
            draw_latex(latex1)
            latex2 = TLatex(0.18,0.75,"%.0f < #it{p}_{T, jet}^{ch} < %.0f GeV/#it{c}" % (self.lvar2_binmin_reco[ibin2],self.lvar2_binmax_reco[ibin2]))
            draw_latex(latex2)
#            latex3 = TLatex(0.18,0.7,"%.1f < #it{z}_{#parallel}^{ch} #leq %.1f" % (self.lvarshape_binmin_reco[0],self.lvarshape_binmax_reco[-1]))
            latex3 = TLatex(0.18,0.7,"%.0f < #it{p}_{T, #Lambda_{c}^{#plus}} < %.0f GeV/#it{c}" % (self.lpt_finbinmin[0],min(self.lpt_finbinmax[-1],self.lvar2_binmax_reco[ibin2])))
            draw_latex(latex3)
