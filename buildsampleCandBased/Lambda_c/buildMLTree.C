...
#include "TChain.h"
using namespace std;
double fMassLc=2.2864;
double fMassLcRange=0.13;
#include <iostream>
...
bool makeMLTree(TString finput,TString foutput,TString treename,int signalsample=1,int maxevents=1000){
  ...
  for(Long64_t i=0;i<maxevents;i++){ 
    ...
    if (signalsample==1){
      if (cand_type==2 || cand_type==3) continue; 
    }
    if (signalsample==0){
      if (fabs(inv_mass-fMassLc)<fMassLcRange) continue; 
    }
