...
// from TTree tree_Bplus/tree_Bplus
...
#ifdef tree_Bplus_cxx
tree_Bplus::tree_Bplus(TTree *tree) : fChain(0) 
{
...
//   if (tree == 0) {
      ...
      // }
      //f->GetObject("tree_Bplus",tree);

      //}
   Init(tree);
}

tree_Bplus::~tree_Bplus()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t tree_Bplus::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Int_t tree_Bplus::GetEntriesFast()
{
  // Read contents of entry.
  if (!fChain) return 0;
  return fChain->GetEntriesFast();
}
Long64_t tree_Bplus::LoadTree(Long64_t entry)
{
...
void tree_Bplus::Init(TTree *tree)
{
   // The Init() function is called when the selector neeBplus to initialize
...
void tree_Bplus::Show(Long64_t entry)
{
...
Int_t tree_Bplus::Cut(Long64_t entry)
{
