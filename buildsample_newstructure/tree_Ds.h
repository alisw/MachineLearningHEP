...
#ifdef tree_Ds_cxx
tree_Ds::tree_Ds(TTree *tree) : fChain(0) 
{
...
//   if (tree == 0) {
      ...
      tree = (TTree*)list2->FindObject();

      //      if (!f || !f->IsOpen()) {
      //   f = new TFile("Memory Directory");
      // }
      //f->GetObject("tree_Ds",tree);
...
Int_t tree_Ds::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Int_t tree_Ds::GetEntriesFast()
{
  // Read contents of entry.
  if (!fChain) return 0;
  return fChain->GetEntriesFast();
}
Long64_t tree_Ds::LoadTree(Long64_t entry)
{
...
void tree_Ds::Show(Long64_t entry)
{
...
Int_t tree_Ds::Cut(Long64_t entry)
{
